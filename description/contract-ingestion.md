...
合同建议名称使用后端阶段 code `file_name_generation`。分类成功后进入该阶段，建议名称成功后再启动字段、条款和相似度三个业务分支；SSE 的阶段完成事件和历史快照 `run.suggested_file_name` 都可恢复名称、命名理由及页码证据。该阶段失败时按服务端 `retryable` 提供独立重试入口。
...
“结果可查看”和“允许正式入库”是不同状态。部分结果可以提前校对，但只有八个用户业务阶段全部为 `succeeded` 时才启用正式入库。成功后保留回执展示并从本地任务列表移除该运行；失败时保留当前审核内容，允许使用同一请求重试。

入库失败提示以高对比度浮层覆盖在结果弹窗底部，不进入审核表单文档流；前端校验与服务端结构化校验返回的多项问题按顺序一次列出，重复文案合并，后端字段路径和校验类型转换为定位到中文业务字段的友好提示，不展示英文错误码。浮层在最后一次更新八秒后自动消失，错误出现和消失不会改变字段、条款或提交按钮的位置。

## 当前交互

- 页面进入后等待用户点击输入节点并选择 PDF，或选择一张 PNG、JPG、WEBP、GIF、BMP、AVIF 图片，不会自动播放处理过程。输入节点始终使用同一套上传卡片：未选择文件时，上方是带云端上传图标的虚线浏览区域，下方是当前文件状态栏；悬停时整卡轻微抬升，上传图标同步上移并增强投影。文件就绪后不切换回旧纸张模板，而是在同一卡片的上区展示封面，底部只保留居中的清除按钮，不再展示左侧文件图标或卡片内改名框；上区及卡片宽度根据封面真实长宽比平滑适配，底部文件栏保持固定高度。文件详情不展示或编辑文件名称；PDF 仍在内部使用上传文件的原始名称创建任务，图片使用转换时自动生成的 PDF 名称。恢复任务也复用同一卡片，以后台占位封面替代不存在的本地图像。
