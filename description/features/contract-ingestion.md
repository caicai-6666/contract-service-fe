...
flowchart LR
    ...
    classification --> overview[合同概述生成]
    overview --> field[字段提取]
    overview --> clause[条款提取]
    overview --> retrieval[相似度处理]
    field --> result[合同提取结果]
    clause --> result
    retrieval --> result
```
...
合同概述生成在分类成功后运行，成功后再启动字段提取、条款提取和相似度处理三个并行阶段。Core 或 Clause 任一分支形成结果后，输出节点立即进入可用状态并展示当前快照中的已有审核分区，不等待其他路径全部完成；相似度处理只参与整体运行状态，单独成功不会开放空白结果面板。八个业务阶段均严格依据服务端 `failed` 与 `retryable` 状态提供断点重试；成功阶段不允许重复执行。重试复用此前已经成功的权威结果，不在前端回退或重跑更早阶段；目标成功后继续消费后续 SSE，并以服务端新结果更新对应审核内容。
...
结果校对区只消费快照顶层 `draft.core` 和 `draft.clauses`，不读取或展示分类、检索问题、向量、文档元数据、证据、推理与内部状态。Core 审核区以定义接口的稳定顺序和字段 `code` 为内部骨架，直接按同名键合并 `draft.core` 的标量、对象或对象数组；界面中的每个 Core 字段只使用定义中的中文名称作为唯一标题，不向审核用户展示英文稳定编码。提取值为 `null`、Core 尚未就绪或某字段没有可靠值时，表单结构仍然完整。`single` 字段在标题下直接提供输入控件，不渲染成列表对象；单属性时不重复显示属性名称，必填和修改状态合并到唯一标题。`multiple` 字段继续以带序号的多项列表呈现，并允许用户新增和移除条目；每项使用独立稳定身份，新增时随占用空间展开并淡入，移除时先收拢再释放空间，操作不会触发结果弹窗关闭。属性类型分别映射为文本、整数、数值或布尔控件；名称为“签订日期”的字符串属性使用年、月、日三个横向排列且彼此独立的紧凑滚轮，兼容横杠、斜杠、点号、中文年月日、八位紧凑日期和 ISO 时间前缀，并统一以 `YYYY-MM-DD` 写回审核草稿，无法解析为真实日历日期的返回值会在正式入库校验中要求用户重新选择。Clause 直接使用 `draft.clauses` 中的条款编号、标题、页码与正文；每条条款顶部按 `path` 顺序纵向展示层级路径，末级加深突出；每级最多展示 24 个字符，超长或容器不足时省略，悬停或键盘聚焦可通过气泡查看完整内容。路径为空时依次回退到 `identifier`、`title` 和条款序号。条款正文默认通过项目统一的安全 Markdown 组件渲染，支持标题、列表、引用、链接、代码块和表格等常用语法，并可在原始 Markdown 编辑与格式化预览间切换；原始 HTML 不执行。条款列表在首部、相邻条款之间和尾部提供插入入口；新增表单由按钮高度平滑展开到实际尺寸，取消或提交时再收拢，使相邻条款与外层分区同步移动。用户逐级填写完整 `path`、起止页码和正文；路径编辑器允许新增或移除层级，并提供 `◆` 作为无常规编号时的快捷前缀。前端从末级路径拆分 `identifier` 和可选 `title`，无法识别常规编号时使用 `◆` 作为 `identifier`；`level` 取路径长度，`parent_clause_id` 通过上一级完整路径匹配已有条款。前端同时生成当前草稿内唯一的 `clause_id`，并按照插入位置重排从 `1` 开始的 `order`。新增条款从零高度和零间距展开至实际尺寸，使后续条款和外层分区连续下移。每条条款均可删除；删除父条款时同步删除全部后代，避免留下无效的父级引用和层级路径，随后重新计算剩余条款顺序。删除过程同时测量并过渡列表容器高度，使条款退场、后续条款补位和外层分区收缩保持同步。页面不再兼容旧的嵌套草稿结构。

最终结果弹窗使用低饱和香槟金表单语言：外壳以细颗粒双层渐变和内外阴影呈现金属厚度，审核分区使用轻微浮起层，输入、选择和条款编辑控件使用内嵌槽与聚焦下压反馈。模型生成的 `run.contract_overview.file_name` 作为最终展示名称初值，审核用户可在入库前修改，且无需填写扩展名；概述尚未形成时，使用上传名称去除 `.pdf` 作为初值。数值字段使用较短的输入区，并在右外侧提供独立的减、加按键；两枚按键嵌入同一固定凹槽底座，圆形按帽在静止时微微浮起、按下时单独沉入底座。布尔字段默认收拢为单行当前值，点击后以不参与容器布局的浮层向下展开“未选择、是、否”玻璃选项，滑块先移动到新选项后再自动收起。文本字段仍占满整行。必填空值不使用单侧阴影，而以沿圆角边缘循环的紫粉色跑马灯提示；系统启用“减少动态效果”时退化为同色静态描边。条款 Markdown 预览保留浅色纸面对比，避免金属背景削弱长文可读性。

合同概述阶段统一使用 `contract_overview_generation`，用于 SSE 的 `stage.code`、快照的 `stages` 键和重试请求；不再映射旧阶段码。成功的 `stage.completed` 一次性返回 `contract_overview`，其中包括 `file_name`、非空且最多 3000 字符的 `summary`、`reasoning` 与物理页码从 1 开始的 `evidence`。前端校验完整对象，不处理摘要 delta，失败事件不展示半成品。断线或事件丢失时以单任务快照 `run.contract_overview` 覆盖恢复；缺失或 null 清除旧概述，不回退读取旧命名对象。运行列表中的 `suggested_file_name` 仍只作为名称字符串展示。

概述详情完整展示建议名称、浅香槟色摘要阅读区、命名依据和页码证据；摘要保留换行，长文在阅读区内滚动。最终“合同提取结果”在名称编辑区之后使用与名称、核心字段一致的分区容器和内嵌输入样式展示摘要编辑区，摘要可编辑并标注入库必填，显示去除首尾空白后的字符数。最终名称与摘要初值取概述建议，分别追踪用户修改；用户修改后，后续 SSE、快照恢复或重试不覆盖其修改，也不要求最终值与建议值相同。概述在分类之后、字段/条款/相似度三个并行分支之前生成，失败时按服务端 `retryable` 提供独立重试。
...
- 合同文档识别、重复性判断、合同分类、合同概述生成和合同提取结果使用加宽详情面板，并提升标题、说明、候选信息、分类结果和审核表单字号；面板宽度仍受工作台可用空间限制，小尺寸窗口不会横向溢出。
...
- 查重审核事件会自动打开候选面板；断线漏掉事件时，通过快照中的运行状态和查重结果恢复该面板。如果事件到达时正在查看其他弹窗，旧弹窗先以右侧为锚点缩回，完整退出后再进入查重面板；如果已经打开查重面板，则保留外壳，仅让旧候选内容淡出并以新结果淡入。用户关闭后再次点击重复性判断节点仍会打开同一候选面板，不重新请求或丢弃候选；有召回候选且无重复时，仅在服务端处于 `awaiting_deduplication_review`、`can_continue=true` 且尚未继续时显示确认按钮；候选即使被模型标为 `different` 仍需核对。确认继续后进入文档结构理解，之后仍可回看候选，但不会再次出现继续按钮。无候选时，后端通过 `run.continued` 携带空 `candidates`、`can_continue=false`、空审核期限及自动推进时间；前端保留结果但不自动弹出审核面板，不发送 continue 请求，回看节点时显示“自动通过 / 无需人工确认”。断线恢复使用快照中的同一组字段区分人工确认与自动通过。
- 合同文档识别、重复性判断、文档结构理解、合同分类、合同概述生成、Core、条款和相似度处理八个阶段均依据服务端 `retryable` 状态提供单独重试；前端不自行推断尝试上限。
...
正式入库仅提交 `file_name`、`summary`、完整 `core` 和 `clauses`。名称先去除首尾空白，再自动去除末尾 `.pdf` 后缀（大小写不敏感，兼容重复后缀），回填名称输入框并按处理后的主体校验；摘要去除首尾空白后提交，必填且分别最多 255、3000 个 Unicode 字符；摘要不由后端自动补齐。空白和超长摘要会进入校验问题侧栏，服务端 422 的摘要错误也可定位编辑区。写入失败保留审核内容以便重试。
...
识别和查重的 `stage.completed` 可能已经携带终态，但详细结果仍由紧随其后的运行事件提供；此时不能仅根据 `overall_status` 关闭 SSE。Core/Clause 同样等待后续草稿事件。若在等待详情期间断流，通过快照恢复状态和候选，而不是停留在只有终止文案的界面。事件分包回归测试见 `tests/extractionEventLifecycle.test.js`；三种交互和快照恢复的状态判定见 `tests/deduplicationReview.test.js`。人工继续的 `run.continued` 若未携带查重结果，前端补读权威快照更新确认状态，保留原候选。
...
- 验证查重暂停、候选鉴权预览、确认继续、八阶段失败断点重试和断线恢复；分别通过实时 SSE 与历史快照确认建议名称、摘要、理由和证据可恢复。两处 PDF 入口使用同一查看器样式，预览开关能正确暂停并恢复分子背景。
